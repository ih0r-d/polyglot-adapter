...
public abstract class BaseExecutor implements AutoCloseable {
    ...
    /// ### VIRTUAL_EXECUTOR
    /// Shared global virtual-thread pool for async tasks.
        ...
        try (InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(cpPath)) {
            if (is != null) {
                try (Reader reader = new InputStreamReader(is, StandardCharsets.UTF_8)) {
                    return Source.newBuilder(lang.id(), reader, cpPath).buildLiteral();
                }
            }
        }
    ...
    public Value evaluate(String code) {
        try {
            return context.eval(Source.newBuilder(languageId(), code, "inline." + languageId()).buildLiteral());
        } catch (Exception e) {
