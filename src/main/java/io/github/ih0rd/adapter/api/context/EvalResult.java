...
public record EvalResult<T>(Class<T> type, T value) {
  ...
  public static <T> EvalResult<T> of(T value) {
    return new EvalResult<>(
        value != null ? (Class<T>) value.getClass() : (Class<T>) Object.class, value);
  }
  ...
  public <R> R as(Class<R> targetType) {
    ...
    if (!targetType.isAssignableFrom(type)) {
      throw new ClassCastException("Cannot cast " + type.getName() + " to " + targetType.getName());
    }
  ...
  public String toString() {
    return "EvalResult[type=%s, value=%s]"
        .formatted(type != null ? type.getSimpleName() : "null", value);
  }
}
