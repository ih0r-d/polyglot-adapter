...
/**
 * Internal executor for Python classes via GraalPy.
 * Implements BaseExecutor contract.
 */
public record PyExecutor(Context context) implements BaseExecutor {
    ...
    public static PyExecutor createDefault() {
        return new PyExecutor(PolyglotContextFactory.createDefault());
    }
    ...
    private <T> T mapValue(Class<T> memberTargetType, String methodName) {
        ...
        if (!checkIfMethodExists(memberTargetType, methodName) || member.getMember(methodName) == null) {
            throw new EvaluationException("Method " + methodName + " is not supported for " + memberTargetType);
        }
        ...
        if (optionalPath.isEmpty()) {
            throw new EvaluationException(
                    "Cannot find Python file: " + pyFileName +
                            " (classpath '" + resourcePath + "' or local '" + PROJ_RESOURCES_PATH + "')");
        }
