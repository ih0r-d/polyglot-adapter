...
class BaseExecutorTest {
    ...
    interface DummyApi { String ping(); }

    static class DummyExecutor extends BaseExecutor {
        DummyExecutor() { super(null, null); }
        public String languageId() { return "js"; }

        @Override protected <T> io.github.ih0rd.adapter.api.context.EvalResult<?> evaluate(String m, Class<T> c, Object... a) {
            return io.github.ih0rd.adapter.api.context.EvalResult.of("ok");
        }
        @Override protected <T> io.github.ih0rd.adapter.api.context.EvalResult<?> evaluate(String m, Class<T> c) {
            return io.github.ih0rd.adapter.api.context.EvalResult.of("ok");
        }
    }
}
