...
/**
 * Optimized Python executor for GraalPy.
 * Extends BaseExecutor to handle Python class bindings and method evaluation.
 */
public final class PyExecutor extends BaseExecutor {
    ...
    @Override
    protected <T> EvalResult<?> evaluate(String methodName, Class<T> memberTargetType, Object... args) {
    ...
    private <T> void validate(String pyClassName, Class<T> memberTargetType) {
        ...
        if (!interfaceName.equals(pyClassName)) {
            throw new EvaluationException(
                    "Interface name '%s' must equal Python class name '%s'".formatted(interfaceName, pyClassName)
            );
        }
    }
    ...
    private <T> Source getFileSource(Class<T> memberTargetType) {
        return SOURCE_CACHE.computeIfAbsent(memberTargetType, cls -> {
